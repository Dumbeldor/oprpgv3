<div class="row pageNormale">
    <h2>Panel de modération</h2><br><br>
    <h2>Gameplay</h2><br>
    <a href="<?= base_url('modo/map/')?>">Gestion de map</a><br>
    <a href="<?= base_url('modo/objets')?>">Gestion objets</a><br>
    <a href="<?= base_url('modo/monster')?>">Gestion monstres</a><br>
    <a href="<?= base_url('modo/techn')?>">Gestion techniques</a><br>
    <a href="<?= base_url('modo/pet')?>">Gestion animaux(pet)</a><br>

    <h2>Communauté</h2><br>
    <a href="<?= base_url('modo/map/createType')?>">Gestion utilisateurs</a><br>
    <a href="<?= base_url('modo/map/createType')?>">Gestion forum</a><br>
</div>