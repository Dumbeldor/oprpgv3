...
$("#etape_cheveux").click(function(){
	...
	$("#options_avatars div#options").empty();
	if(sexe == 'Homme') {
		for(var i=1;i<=nb_cheveux_homme;i++) {
			$("#options_avatars div#options").append('<a><img class="option_avatar" id="hair-'+i+'" src="'+avat_dir+'/man/hair/'+i+'.png"></a> ');
		}
	} else {
...
$("#etape_yeux").click(function(){
	$("#options_avatars div#options").empty();
	for(var i=1;i<=nb_yeux;i++) {
		$("#options_avatars div#options").append('<a><img class="option_avatar" id="eyes-'+i+'" src="'+avat_dir+'/eyes/'+i+'.png"></a> ');
	}
});
...
function avatar_construct() {
	...
	if(hair != "0") {
		var sexe = $('input[name=sexe]:checked').val();
		var folder = (sexe == 'Homme') ? 'man' : 'woman';
		$("div#currentAvatarPanel").append('<img class="currentAvatar" src="'+avat_dir+'/'+folder+'/hair/'+hair+'.png">');
	}
	
	if(eyes != "0") {
		$("div#currentAvatarPanel").append('<img class="currentAvatar" src="'+avat_dir+'/eyes/'+eyes+'.png">');
	}
